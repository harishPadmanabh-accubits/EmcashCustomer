...
class WalletActivityDetailsAdapter(val data: List<WalletActivity>) :
    RecyclerView.Adapter<WalletActivityDetailsAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.itemView.apply {
            tv_time.text = currentItem.transactionInfo.updatedAt.toLocalTime()
            val type = currentItem.transactionInfo.type
            if (type == PAYMENT_TYPE_TRANSFER) {
                if (currentItem.mode == PAYMENT_MODE_CREDIT) {
            ...
            tv_balance.apply {
                text = currentItem.balance.toString()
                if (currentItem.mode == PAYMENT_MODE_CREDIT) {
